...
public class ShapeEllipsoid extends Shape{
	...
	private PropertyPositiveInt propertySemiX = new PropertyPositiveInt(0, 165, 3, new TranslationTextComponent("property.buildguide.semiaxis", "X"), () -> {this.update();});
	private PropertyPositiveInt propertySemiY = new PropertyPositiveInt(0, 185, 3, new TranslationTextComponent("property.buildguide.semiaxis", "Y"), () -> {this.update();});
	private PropertyPositiveInt propertySemiZ = new PropertyPositiveInt(0, 205, 3, new TranslationTextComponent("property.buildguide.semiaxis", "Z"), () -> {this.update();});
	private PropertyEnum<dome> propertyDome = new PropertyEnum<dome>(0, 225, dome.NO, new TranslationTextComponent("property.buildguide.dome"), () -> {this.update();}, domeNames);
			...
			for(int y = propertyDome.value == dome.POSITIVE_Y ? 0 : -dy; y <= (propertyDome.value == dome.NEGATIVE_Y ? 0 : dy);++y) {
				for(int z = propertyDome.value == dome.POSITIVE_Z ? 0 : -dz; z <= (propertyDome.value == dome.NEGATIVE_Z ? 0 : dz);++z) {
					...
					if(r2_outer >= 1 && r2_inner <= 1) {
						addCube(builder, x + 0.2, y + 0.2, z + 0.2, 0.6);
					}
				}
			}
		}
