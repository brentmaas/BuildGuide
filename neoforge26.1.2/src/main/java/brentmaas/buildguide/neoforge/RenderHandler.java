...
public class RenderHandler extends AbstractRenderHandler {
	private static final RenderPipeline.Snippet BUILD_GUIDE_SNIPPET = RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
			.withColorTargetState(new ColorTargetState(new BlendFunction(SourceFactor.SRC_ALPHA, DestFactor.ONE_MINUS_SRC_ALPHA, SourceFactor.SRC_ALPHA, DestFactor.ONE_MINUS_SRC_ALPHA)))
			.withCull(true)
			.buildSnippet();
	...
	@SubscribeEvent
	public void onRenderBlock(RenderLevelStageEvent.AfterWeather event) {
		render();
	}
